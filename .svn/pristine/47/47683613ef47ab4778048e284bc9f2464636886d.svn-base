...
void pids(char *path, int names)
{
    	...
    	if(is_proc(pid))
    	{
    		...
    		while((sub_dir_ptr = readdir(sub_dir)) != NULL)
    		{
    			...
    			char *name = sub_dir_ptr->d_name;
    				...
    				found_contents[size_contents - 1] = (char*)calloc((strlen(pid) + size_str + 1), sizeof(char));
    				strcpy(found_contents[size_contents - 1], pid);
    				strcat(found_contents[size_contents - 1], " ");
    				strncat(found_contents[size_contents - 1], buff, size_str);
    ...
    else if(names) 
    {
    	...
    	for(int i = 0; i < size_contents; i++) 
    	{
	    	printf("%s of size %lu\n", found_contents[i], strlen(found_contents[i]));
   		}	
    }
