    	...
    	if(is_proc(pid))
    	{
    		...
    		while((sub_dir_ptr = readdir(sub_dir)) != NULL)
    		{
    			...
    			char *name = sub_dir_ptr->d_name;
 	   			if(!strcmp(name, "comm")) {
 	   				char *comm_path = calloc((strlen(sub_path) + 5), sizeof(char));
 	   				strcpy(comm_path, sub_path);
 	   				strcat(comm_path, "/");
    				...
    				strcat(found_contents[size_contents - 1], buff);
...
void ps(char *path)
{
    			...
    			char *name = sub_dir_ptr->d_name;
    				...
    				int fd = open(status_path, O_RDONLY);
 					...
 					while(*(buff + buff_index) != '\n') {
 						name = (char*)realloc(name, (++size_name)*sizeof(char));
 						name[size_name - 1] = *(buff + buff_index);
 						buff_index++;
 					}
 					...
 					size_stat = 0;
 					while(*(buff + buff_index) != ' ') {
 						status = (char*)realloc(status, (++size_stat)*sizeof(char));
 						status[size_stat - 1] = *(buff + buff_index);
 						buff_index++;
 					}
					...
					buff_index += 5;
 					...
 					size_pid = 0;
 					while(*(buff + buff_index) != '\n') {
 						pid = (char*)realloc(pid, (++size_pid)*sizeof(char));
 						pid[size_pid - 1] = *(buff + buff_index);
 						buff_index++;
 					}
	...
	for(int i = 0; i < size_contents; i++) {
		...
		printf("%6s\n ", extract_state(found_contents[i]));
...
void sort_found_contents(char **found_contents, int size_contents)
{
	for(int i = 0; i < size_contents - 1; i++)
	{
		...
		for(int j = i + 1; j < size_contents; j++)
		{
			...
			if(strcmp(found_contents[min_index] + index_start_name_fst, found_contents[j] + index_start_name_snd) > 0)
			{
				min_index = j;
			}
			else if(strcmp(found_contents[min_index] + index_start_name_fst, found_contents[j] + index_start_name_snd) == 0)
			{
