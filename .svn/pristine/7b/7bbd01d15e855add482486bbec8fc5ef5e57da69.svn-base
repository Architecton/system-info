...
void construct_tree(Node root, Node prev, int *input_data, int input_data_size, int num_children, int child_parse_index)
{
	...
	for(int i = 0; i < num_children; i++)
	{
		...
		int follow_up = 0;
		for(int j = child_parse_index + i; j < num_children_child; j++)
		{
			follow_up += input_data[i];
		}

		int parse_start_index = child_parse_index + num_children + input_data[child_parse_index] + follow_up;
